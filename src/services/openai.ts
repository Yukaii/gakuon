...
export class OpenAIService {
  ...
  private validateFields(card: Card, deckConfig: DeckConfig): void {
    ...
    const fieldReferences = [...deckConfig.prompt.matchAll(/\${([^}]+)}/g)]
      .map(match => match[1]);
  ...
  private replaceFieldReferences(prompt: string, card: Card, deckConfig: DeckConfig): string {
    let result = prompt;
    const fieldReferences = [...prompt.matchAll(/\${([^}]+)}/g)]
      .map(match => match[1]);
...
${Object.entries(deckConfig.responseFields)
  .map(([field, config]) => `- ${field}: ${config.description}${config.required ? ' (required)' : ' (optional)'}`)
  .join('\n')}`;
  }

  async generateContent(card: Card, deckConfig: DeckConfig): Promise<DynamicContent> {
    try {
      ...
      const response = JSON.parse(completion.choices[0].message.content!) as Record<string, string>;
