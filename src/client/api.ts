...
export async function fetchDeckCards(deckName: string): Promise<Card[]> {
  const response = await fetch(`${API_BASE}/decks/${encodeURIComponent(deckName)}/cards`);
...
export async function answerCard(cardId: number, answer: CardAnswer): Promise<{ success: boolean }> {
...
export async function regenerateCard(cardId: number): Promise<{ content: Record<string, string> }> {
